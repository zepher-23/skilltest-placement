...
const changeStatus = async (req, res) => {
    ...
    const resultExists = await Result.findOne({ $and: [{ interview: interviewId }, { student: studentId }] })
    if (resultExists) {
      const updateResult = await Result.findOneAndUpdate({ $and: [{ interview: interviewId }, { student: studentId }] },{$set:{result}},{new:true})
        if (updateResult) {
